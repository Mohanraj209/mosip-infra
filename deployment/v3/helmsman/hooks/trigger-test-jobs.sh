...
# Function to trigger all CronJobs in a namespace
trigger_all_cronjobs_in_namespace() {
    ...
    # Track job names for later checking
    job_names=()
    
    # Trigger each CronJob
    ...
    if [[ ${#job_names[@]} -eq 0 ]]; then
        echo "WARNING: No jobs were created in namespace $namespace"
        return 0
    fi
    
    # Wait for all jobs to complete
    local result
    wait_for_jobs $namespace result "${job_names[@]}"
    return $result
}
...
trigger_specific_dsl_cronjob() {
    ...
    if kubectl get cronjob -n $namespace $cronjob &>/dev/null; then
        ...
        local result
        wait_for_jobs $namespace result "$job_name"
        return $result
    else
    ...
    while [[ ${#unfinished_jobs[@]} -gt 0 ]]; do
        ...
        for job_name in "${unfinished_jobs[@]}"; do
            ...
            if [[ "$job_status" == "True" ]]; then
                ...
                # Optionally, you could delete the job here to force it to stop
...
# Step 1: Trigger all CronJobs in apitestrig namespace
echo "=== STEP 1: Processing apitestrig namespace ==="
...
# Step 2: Trigger specific CronJob in dslrig namespace
echo "=== STEP 2: Processing dslrig namespace ==="
trigger_specific_dsl_cronjob
dslrig_result=$?
...
# Step 3: Trigger all CronJobs in uitestrig namespace
